...
export function StudentMetrics({ data }: StudentMetricsProps) {
  ...
  const active_students = db_students.filter((student) => student.status);
  ...
  const not_logged_in = active_students.filter(
    (student) =>
      student.lms_activity_w4 && student.lms_activity_w4.last_login_at === null
  );
  ...
  const zero_progress = active_students.filter(
    (student) =>
      student.lms_activity_w4 &&
      student.lms_activity_w4.course_progress === 0 &&
 ...
 */
  const low_progress = active_students.filter(
    (student) =>
      student.lms_activity_w4 &&
      student.lms_activity_w4.course_progress <= 0.2 &&
